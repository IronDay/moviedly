...
rentalRoutes.post("/", async (req, res) => {
    ...
    const fetchedMovie = await Movie.findById(body.movie);
    if (!fetchedMovie) return res.status(400).send("Movie not found.");

    const fetchedCustomer = await Customer.findById(body.customer);
    if (!fetchedCustomer) return res.status(400).send("Customer not found. Register first please");

    let rental = new Rental({...body, movie: fetchedMovie, customer: fetchedCustomer});
...
rentalRoutes.put("/:id", async (req, res) => {
    ...
    Rental.findByIdAndUpdate({_id: req.params.id}, {
        $set: {
            ...req.body,
            movie: fetchedMovie,
